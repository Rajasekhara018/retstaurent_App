...
@Component({
  selector: 'app-head-quaters',
  templateUrl: './head-quaters.component.html',
  styleUrls: ['./head-quaters.component.scss']
})
export class HeadQuatersComponent implements OnInit {
  reviewData: any;
  registerDetails: any;
  homeDetails: any;
  starData: any;
  ...
  showReviews(){
    if(JSON.parse(localStorage.getItem('reviewData')) && JSON.parse(localStorage.getItem('tableRegistered'))){
      ...
      if(this.reviewData[1] == "HeadQuaters"){
